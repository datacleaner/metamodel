...
import org.datacleaner.metamodel.QueryPostprocessDataContext;
...
import org.datacleaner.metamodel.util.FileResource;
import org.datacleaner.metamodel.util.ImmutableRef;
import org.datacleaner.metamodel.util.NumberComparator;
import org.datacleaner.metamodel.util.Resource;
import org.datacleaner.metamodel.util.UrlResource;
...
public class XmlDomDataContext extends QueryPostprocessDataContext {
    ...
    private void loadSchema(Document document) {
        ...
        Table[] tables = _schema.getTables().toArray(new Table[0]);
    ...
    private void loadTables(Element element, String tablePrefix, Column parentKeyColumn, int parentKey) {
        ...
        if (attributes.length > 0 || textContent != null || hasSiblings(element)) {
            ...
            if (table == null) {
                ...
                Column[] foreignKeys = table.getForeignKeys().toArray(new Column[0]);
    ...
    public XmlDomDataContext flattenTables(Relationship relationship) {
        ...
        List<Column> primaryColumns = new ArrayList<Column>(primaryTable.getColumns());
        List<Column> foreignColumns = new ArrayList<Column>(foreignTable.getColumns());
        ...
        Column[] foreignKeys = foreignTable.getForeignKeys().toArray(new Column[0]);
            ...
            if (_tableData.containsKey(table.getName())) {
                ...
                if (foreignKeyRelationships.length == 1 && table.getPrimaryKeyRelationships().size() == 0) {
                    ...
                    if (columns.length == nonDataColumns + 1) {
                        ...
                        Column[] foreignColumns = foreignKeyRelationship.getForeignColumns().toArray(new Column[0]);
